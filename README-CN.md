...
#### v0.5.5
* 📶 **蓝牙稳定性提升**：
  * **幂等启动控制**：通过在启动前校验 BLE 监听器状态，修复了 `0xffffffff8000000e` 异常（Start adv watcher exception）。
  * **消除启动竞态**：解决了程序启动时由于初始连接事件触发的多重扫描器启动冲突。
  * **代码修复**：修正了 `DeviceWatcher` 状态校验逻辑中的笔误（原代码重复校验了 `Started` 状态）。
